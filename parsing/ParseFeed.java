...
public class ParseFeed {
	...
	public static HashMap<String, Float> LoadDisastersFromCSV(PApplet p, String fileName){
		HashMap<String, Float> disasterMap =  new HashMap<>();
		...
		for (String row : rows) {
			...
			for(int i = 0; i < columns.length; i++) {
				try {
					if(!columns[i].equals("..")) {
						disasterMap.put(columns[0], Float.parseFloat(columns[3]) * 1000000);
					}
				} catch(Exception e) {
					continue;
				}
